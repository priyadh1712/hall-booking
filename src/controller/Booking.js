...
let rooms = [
  ...
  {
    room_id: 3,
    room_name: "Royal Banquet Hall",
    room_status: "available",
    amenities: "Stage, Sound System, Decorative Lighting",
    seats: 50,
    price_per_hrs: 5000,
  },
];
...
const bookRoom = async (req, res) => {
  try {
    const { customer_name, start_time, end_time, roomID } = req.body;
    const date = format(new Date(), "dd-MM-yyyy");

    // Check if the room is already booked on the same date and time
    const isConflict = bookingRoom.some((booking) => {
      // Check if the room ID matches and the date matches
      if (booking.roomID === roomID && booking.Date === date) {
        // Check for overlapping times
        const isStartTimeConflict = start_time < booking.end_time && start_time >= booking.start_time;
        const isEndTimeConflict = end_time > booking.start_time && end_time <= booking.end_time;
        const isFullOverlap = start_time <= booking.start_time && end_time >= booking.end_time;

        return isStartTimeConflict || isEndTimeConflict || isFullOverlap;
      }
      return false;
    });
